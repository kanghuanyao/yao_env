...
char shortopt[]="hl:p:a:d:D:u:L:fP:t:";
...
static void handlemessage(struct dhcp_msg*rmsg)
{
	...
	if(prefixcnt && (j=messagefindoption(rmsg,OPT_IAPD))>=0){
		...
		pref.opt_iaprefix.preferred_lifetime=ptime;
		pref.opt_iaprefix.valid_lifetime=ptime*2;
	...
	if(addresscnt && (j=messagefindoption(rmsg,OPT_IANA))>=0){
		...
		addr.opt_iaaddress.preferred_lifetime=ptime;
		addr.opt_iaaddress.valid_lifetime=ptime*2;
...
int main(int argc,char**argv)
{
        ...
        while(1){
                ...
                switch(c){
                        ...
                        case 'P':pidfile=optarg;break;
												case 't':ptime=(unsigned int)atoi(optarg);break;
                        default:
	...
	addrecvfilter(MSG_RENEW);
	addrecvfilter(MSG_REBIND);
