...
// ARP Header
	...
	byte1 proto_size;
	byte2 arp_opcode;	//8
	byte1 sender_mac[6];	//14
	byte4 sender_ip;	//18
	byte1 target_mac[6];	//24
	byte4 target_ip;	// 28
// Paddign
	char padding[18];
	char padding2[1500-60+14];
}ARP_PKT;
...
int main(int argc, char *argv[])
{
	...
	unsigned long int pkt_size=0;

	if( argc != 3 )
	{
		printf("Usage: ./arp INTERFACE IPADDRESS\n");
		exit(1);
	}
	...
	memset(&(pkt.padding2), 0, 1500-60+14);
